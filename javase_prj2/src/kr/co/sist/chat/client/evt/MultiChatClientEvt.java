...
public class MultiChatClientEvt extends WindowAdapter implements ActionListener, Runnable {
	...
	public void capture() throws IOException {
		...
		switch(JOptionPane.showConfirmDialog(mccv, "대화내용을 저장 하시겠습니까?")) {
			//스트림을 사용하여 저장
		case JOptionPane.OK_OPTION :
			...
			try {
				...
				JOptionPane.showMessageDialog(mccv, saveDir+"에 대화내용 기록되었습니다.");
			}finally {
				if(bw!=null) {bw.close();};
			}
		...
		if (ae.getSource() == mccv.getJbtCapture()) {
			try {
				if(!mccv.getJtaTalkDisplay().getText().equals("")) {
					...
					JOptionPane.showMessageDialog(mccv, "저장할 파일이 없습니다.");
				}
			} catch (IOException ie) {
