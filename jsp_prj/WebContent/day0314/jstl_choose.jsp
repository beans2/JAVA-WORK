...
<%-- 	<%
	...
	<c:choose>
		...
		<c:when test="${tooltip eq 1 }">
			<c:set var="tooltipId" value="show-event"/>
		</c:when>
		<c:otherwise>
			<c:set var="tooltipId" value="hide-event"/>
		</c:otherwise>
	</c:choose>
	
	<div style="width: 500px; height: 200px; overflow: auto; border:1px solid #333">
		1조는 PC방 프로그램을 아이템으로 구현하였고, 조장 
		<span class="${tooltipId}" title="좌석,메시지 담당">이봉현</span>조원
		<span  class="${tooltipId}"title="주문,통계 담당">이재찬</span>,
		<span  class="${tooltipId}"title="회원관리, 관리자관리">김정운</span>,
		...
		PC방 프로그램을 완벽하게 구현하기 위해서 조장이하 모든 조원이 합심하여 PC방으로 전격출근!!!!
		알바를 하였습니다.
	</div>
