...
public class DiaryDAO {
	...
	/**
	 * 년,월을 입력받아 해당 월의 모든 일자의 글번호, 제목을 가변배열 에 저장하여
	 * 반환하는 일
	 * @param year 년
	 * @param month 월
	 * @return 가변배열
	...
	public MonthVO[][] selectMonthEvent(String year,String month) throws SQLException{
		...
		//5.
			...
			for(int i=0; i< 32;i++) { //해당 년월의 
				pstmt.setString(3,String.valueOf(i+1)); //1일부터 31일까지 쿼리수행
				rs= pstmt.executeQuery();
				while(rs.next()) {//실행 결과가 존재한다면 해당 일자의 이벤트 글이 존재하므로 
					//글의 값을 저장한다.
	...
	/**
	 * 글번호, 비밀번호를 입력받아 해당 글의 삭제 하는 일.
	 * @param dr_vo
	 * @return
	 * @throws SQLException
	 */
	public int delectEvent(DiaryRemoveVO dr_vo) throws SQLException{
		int cnt=0;
		Connection con=null;
		PreparedStatement pstmt = null;
	...
	public int selectEvtCnt(SearchDataVO sd_vo)throws SQLException{
		...
		//4.
			...
			selectCnt
			.append(" select count(*) cnt from diary ");
			if(sd_vo!=null) {
				//Dynamic Query
	...
	public List<DiaryListVO> selectList(ListRangeVO lr_vo,SearchDataVO sd_vo)throws SQLException{
		...
		//4.
			...
			selectList
			.append(" select r_num,num, writer, subject, e_year, e_month, e_day, to_char(w_date,'yyyy-mm-dd hh24:mi') w_date ")
			.append(" from(select num, writer, subject, e_year, e_month, e_day, w_date, ")
			...
			while(rs.next()) {
				dl_vo=new DiaryListVO(rs.getInt("num"), rs.getString("subject"),rs.getString("writer"), 
						rs.getString("e_year"),rs.getString("e_month"), rs.getString("e_day"), rs.getString("w_date"));
				list.add(dl_vo);
			}//end while
