...
public class UseWebParameter extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=  response.getWriter();
	      ...
	      out.println("\t<div>");
	      out.println("\t<h2>GET방식으로 요청한 파라메터 값처리</h2>");
	      out.println("\t<div>");
	      out.println("\t<ul>");
	      
	      out.print("\t<li>이름: <strong>");
//	      out.print(HangulConv.toUTF(name)); Tomcat 8.x이하일때는 한글처리를 해야한다.
	      out.print(name);
	      ...
	      out.print(mailing); //value 속성이 없는 checkbox는 체크되면 on, 체크되지 않으면 null
	      out.println("</strong></li>");
...
//	      out.print(HangulConv.toUTF(greeting));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=  response.getWriter();
	      ...
	      out.println("\t<div>");
	      out.println("\t<h2 style='color:#0000FF'>POST방식으로 요청한 파라메터 값처리</h2>");
	      out.println("\t<div>");
	      out.println("\t<ul>"); 
